...
class KnowledgeAnalyzer:
    ...
    def create_research_tasks(self, all_entries: Dict[str, CatalogEntry]):
        """Create research tasks in research-tasks.md from detected gaps."""
        print(f"\n{CYAN}📝 Creating research tasks from gaps...{NC}")

        # Collect gaps by priority
        gaps_by_priority = {
            "blocking": [],
            "high": [],
            "medium": [],
        ...
        for doc_path, entry in all_entries.items():
            for gap in entry.gaps:
                ...
                gaps_by_priority[priority].append((doc_path, gap))

        # Read existing research-tasks.md
        research_tasks_path = self.repo_root / "research-tasks.md"
        if research_tasks_path.exists():
            existing_content = research_tasks_path.read_text()
        else:
            existing_content = ""
        ...
        # High priority gaps
        if gaps_by_priority["high"]:
            new_tasks.append("## High Priority\n")
            new_tasks.append("_(Tasks that impact multiple agents or critical workflows)_\n\n")

            for doc_path, gap in gaps_by_priority["high"]:
                doc_name = Path(doc_path).stem
                category = Path(doc_path).parts[0]  # services, providers, etc.

                new_tasks.append(f"### {gap['topic'].replace('_', ' ').title()} - {doc_name}\n")
                new_tasks.append(f"**Gap ID:** {gap['gap_id']}\n")
                new_tasks.append(f"**Type:** {gap['gap_type']}\n")
                new_tasks.append(f"**Document:** {doc_path}\n")
                new_tasks.append(f"**Description:** {gap['description']}\n\n")
                new_tasks.append("**Suggested research:**\n")

                # Generate context-specific research suggestions
                if gap['topic'] == 'deployment':
                    new_tasks.append(f"- Check argocd manifests for {doc_name}\n")
                    new_tasks.append(f"- Review {category}/{doc_name} deployment configuration\n")
                    new_tasks.append("- Document scaling and resource requirements\n")
                elif gap['topic'] == 'authentication':
                    new_tasks.append(f"- Review auth implementation in server/{doc_name}\n")
                    new_tasks.append("- Document supported auth methods\n")
                    new_tasks.append("- Include auth flow diagrams if complex\n")
                elif gap['topic'] == 'rate_limiting':
                    new_tasks.append(f"- Check rate limiting implementation in {doc_name}\n")
                    new_tasks.append("- Document actual rate limits and strategies\n")
                    new_tasks.append("- Note per-endpoint vs global limits\n")
                else:
                    new_tasks.append(f"- Investigate {gap['topic']} in {doc_name}\n")
                    new_tasks.append("- Document current implementation\n")
                    new_tasks.append("- Include examples and best practices\n")

                new_tasks.append(f"\n**Auto-generated by knowledge-analyzer:** {datetime.now().strftime('%Y-%m-%d')}\n\n")

        # Medium priority gaps
        if gaps_by_priority["medium"]:
            new_tasks.append("## Medium Priority\n")
            new_tasks.append("_(Useful but not blocking)_\n\n")

            # Limit to first 10 medium priority to avoid overwhelming
            for doc_path, gap in gaps_by_priority["medium"][:10]:
                doc_name = Path(doc_path).stem
                new_tasks.append(f"### {gap['topic'].replace('_', ' ').title()} - {doc_name}\n")
                new_tasks.append(f"**Gap ID:** {gap['gap_id']}\n")
                new_tasks.append(f"**Description:** {gap['description']}\n")
            ...
            if len(gaps_by_priority["medium"]) > 10:
                remaining = len(gaps_by_priority["medium"]) - 10
                new_tasks.append(f"_...and {remaining} more medium priority gaps (see catalog for details)_\n\n")

        # Update research-tasks.md
        # Insert new tasks after the "High Priority" header
        if "## High Priority" in existing_content:
            # Replace existing High Priority section
            parts = existing_content.split("## High Priority")
            header = parts[0]

            # Find end of Medium Priority section (or end of Low Priority)
            if len(parts) > 1:
                rest = parts[1]
                if "## Low Priority" in rest:
                    low_priority_parts = rest.split("## Low Priority")
                    # Keep Low Priority and everything after
                    footer = "## Low Priority" + low_priority_parts[1]
                else:
                    footer = ""
            else:
                footer = ""

            new_content = header + "".join(new_tasks) + footer
        else:
            # Append to end
            new_content = existing_content.rstrip() + "\n\n" + "".join(new_tasks)

        # Write back
        research_tasks_path.write_text(new_content)

        total_created = len(gaps_by_priority["high"]) + min(10, len(gaps_by_priority["medium"]))
        print(f"  ✅ Created {total_created} research tasks in research-tasks.md")
        print(f"     - {len(gaps_by_priority['high'])} high priority")
        print(f"     - {min(10, len(gaps_by_priority['medium']))} medium priority (limited to first 10)")

    def auto_invoke_researcher(self, all_entries: Dict[str, CatalogEntry]):
        """Auto-invoke researcher agent for blocking gaps."""
        print(f"\n{CYAN}🤖 Checking for blocking gaps (auto-invoke researcher)...{NC}")

        # Collect blocking gaps
        blocking_gaps = []
        for doc_path, entry in all_entries.items():
            for gap in entry.gaps:
                if gap["priority"] == "blocking":
                    blocking_gaps.append((doc_path, gap))

        if not blocking_gaps:
            print(f"  {GREEN}✓{NC} No blocking gaps detected (auto-invoke not needed)")
            return

        print(f"  {RED}!{NC} Found {len(blocking_gaps)} blocking gap(s)")

        for doc_path, gap in blocking_gaps:
            print(f"\n  {YELLOW}▶{NC} Auto-invoking researcher for: {gap['gap_id']}")
            print(f"     Topic: {gap['topic']}")
            print(f"     Document: {doc_path}")

            # Note: Actual researcher invocation would use Task tool here
            # For now, we just log that it would be invoked
            # In production, this would be:
            # Task(
            #   subagent_type="knowledge-researcher",
            #   prompt=f"Research and document {gap['topic']} for {doc_path}. Context: {gap['description']}"
            # )

            print(f"     {CYAN}→{NC} Would invoke: Task(knowledge-researcher, topic={gap['topic']})")
            print(f"     {YELLOW}NOTE:{NC} Auto-invoke not yet fully wired (see TECH-DEBT.md)")
    ...
    def run_full_scan(self):
        ...
        self.create_research_tasks(all_entries)

        # Auto-invoke researcher for blocking gaps
        self.auto_invoke_researcher(all_entries)
